...
from struct_roberta import StructRoberta
...
def model_load(fn, model, device):
    ...
    # strict=False allows loading weights even if architecture has changed slightly
...
def evaluate(model, loader, device):
    ...
    for i, (data, heads) in enumerate(loader):
        ...
        outputs = model(input_data, targets=targets)
        loss = outputs["loss"]
...
def train(args, model, optimizer, scheduler, train_loader, valid_loader, device):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for data, heads in train_loader:
            ...
            outputs = model(input_data, targets=targets)
            loss = outputs["loss"]
            if torch.isnan(loss) or loss is None: # Added check for None loss
                print(f"Warning: Invalid loss at step {global_step}. Skipping update.")
                continue
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Masked LM training with Struct-RoBERTa')
    parser.add_argument('--base_lr', type=float, default=1e-5)
    parser.add_argument('--parser_lr', type=float, default=1e-5)
    parser.add_argument('--train_file', type=str, required=True)
    parser.add_argument('--valid_file', type=str, required=True)
    parser.add_argument('--tokenizer_path', type=str, default='roberta_hi_tok')
    parser.add_argument('--model_path', type=str, default='roberta_hi')
    ...
    parser.add_argument('--save', type=str, default='struct_roberta_final.pt')
    parser.add_argument('--checkpoint_path', type=str, default='roberta_checkpoint.pt')
    parser.add_argument('--validation_interval', type=int, default=2500)
    parser.add_argument('--resume', type=str, default='')
    parser.add_argument('--num_workers', type=int, default=0)
    parser.add_argument('--num_front_layers', type=int, default=3, help='Number of initial layers to keep from the base model.')
    args = parser.parse_args()
    
    device = torch.device('cuda' if args.cuda and torch.cuda.is_available() else 'cpu')
    random.seed(args.seed); np.random.seed(args.seed); torch.manual_seed(args.seed)
    if torch.cuda.is_available() and args.cuda: torch.cuda.manual_seed(args.seed)
    ...
    model = StructRoberta(model_name=args.model_path, num_front_layers=args.num_front_layers)
    model.to(device)

    print("Setting up optimizer with differential learning rates...")
    new_component_names = {n for n, p in model.parser.named_parameters()}
    if model.rear_layers:
        new_component_names.update(n for n, p in model.rear_layers.named_parameters() if "struct_gate" in n)
    
    base_params = [p for n, p in model.named_parameters() if n not in new_component_names]
    new_params = [p for n, p in model.named_parameters() if n in new_component_names]

    optimizer_grouped_parameters = [
        {"params": base_params, "lr": args.base_lr},
        {"params": new_params, "lr": args.parser_lr},
    ]
