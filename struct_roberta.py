...
class StructRoberta(nn.Module):
    def __init__(self, model_name="roberta-base", num_front_layers=6):
        super().__init__()
        base_mlm_model = RobertaForMaskedLM.from_pretrained(model_name)
        self.base_model = base_mlm_model.roberta
        self.config = self.base_model.config
        self.embeddings = self.base_model.embeddings
        all_layers = self.base_model.encoder.layer
        
        # --- FIX: Use the num_front_layers argument for the split ---
        ...
        self.output_layer = base_mlm_model.lm_head
